# Pre-publish checklist — GT-STACER 26.10 stable

> **26.10 (2026-08-19)** — released via `scripts/release.sh`
> (tag `GT-STACER_26.10_STABLE`). See **[`PUBLISHING.md`](PUBLISHING.md)** for the
...
## Code
- [x] `CMakeLists.txt` → `VERSION 26.10.0`, `APP_VERSION="26.10"`,
      `APP_CHANNEL="stable"`
- [x] Packaging scripts (`build-deb.sh`, `build-rpm.sh`, `build-appimage.sh`,
      `build-all.sh`) use `VERSION="26.10"`
...
- [x] `GT-STACER-26.10-x86_64.AppImage` — includes Wayland plugin
- [x] `GT-STACER_26.10_amd64.deb` — Depends includes `libqt6dbus6` + `libqt6network6`, no `libqt6charts6`
- [x] `gt-stacer-26.10-2.x86_64.rpm` — built via alien from DEB
- [x] `GT-STACER-26.10-x86_64.flatpak` — `org.gnutux.gt-stacer` on `org.kde.Platform//6.9`
- [x] `SHA256SUMS.txt` — covers all four
- [x] `RELEASE_NOTES_26.10.md` Downloads table filled with real sizes + SHA-256
...
- [ ] `flatpak install --user release/GT-STACER-26.10-x86_64.flatpak` succeeds
...
## Documentation
- [x] `CHANGELOG.md` has a `[26.10-stable]` entry dated 2026-08-19
- [x] `RELEASE_NOTES_26.10.md` exists and matches the changelog
- [x] `README.md` shows the 26.10 badge + "What's new" + new comparison rows (AR + EN)
- [x] `SECURITY.md` lists 26.10 stable as current
...
- [x] `packaging/flatpak/org.gnutux.gt-stacer.metainfo.xml` has a `<release version="26.10">` entry
- [x] Website (`GT-STACER-WEB/`) version refs all show 26.10, with screenshots for the 3 new pages
...
```bash
gh release create GT-STACER_26.10_STABLE --repo SalehGNUTUX/GT-STACER \
  --title "GT-STACER 26.10 STABLE" --notes-file RELEASE_NOTES_26.10.md \
  --latest --target main \
  release/GT-STACER-26.10-x86_64.AppImage release/GT-STACER_26.10_amd64.deb \
  release/gt-stacer-26.10-2.x86_64.rpm release/GT-STACER-26.10-x86_64.flatpak \
  release/SHA256SUMS.txt
```

> Tag is **`GT-STACER_26.10_STABLE`** (matches the website's download URLs), NOT
> `v26.10-stable`. Do **not** use `scripts/publish-release.sh` — it uses the
...
- [ ] Open the next milestone (`v26.10 — backup & snapshots`).
